...
//		str1.forEach(System.out::println); //forEach is the terminal opn
		
		//numbers.stream().distinct().forEach(System.out::println); //distinct is intermediate opn
		
		//System.out.println(numbers.stream().distinct().count()); //count is terminal opn
		
		//System.out.println(numbers.stream().filter(n -> n > 5).count()); //filter is intermediate opn
		
		//numbers.stream().distinct().sorted().forEach(System.out::println); //sorted is intermediate
		
		//System.out.println(numbers.stream().reduce(0, (n,m) -> n+m )); //reduce is terminal opn
